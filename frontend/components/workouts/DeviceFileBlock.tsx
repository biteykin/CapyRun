...
type WF = {
  ...
  source: string | null; // 'upload' | 'sync' | ...
  device_vendor?: string | null;
  device_model?: string | null;
  device_product?: string | null;
  device_serial?: string | null;
  format?: string | null; // fit/gpx/tcx
  ext?: string | null;
};
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("workout_files")
        .select(
          "id, original_filename, size_bytes, uploaded_at, source, device_vendor, device_model, device_product, device_serial, format, ext"
        )

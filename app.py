# app.py — CapyRun FIT Analyzer v3.1-stable (no ICS)
# - Single workout report: metrics, charts, zones, tables, Excel
# - Multi-workout dashboard: summaries, ATL/CTL/TSB, 7-day draft plan, Excel
# - No GPS stored/shown; safe dtype handling for dates
...
# ---------------- UI / Sidebar ----------------
st.set_page_config(page_title="CapyRun — FIT Analyzer v3.1-stable", page_icon="🏃", layout="wide")
st.title("🏃 CapyRun — FIT Analyzer v3.1-stable")
...
def parse_fit_file(uploaded_file):
    ...
    if not df_rec.empty:
        ...
        df_rec["dt_s"] = df_rec["t_rel_s"].diff().fillna(0).clip(lower=0)  # non-negative time steps
    ...
    if not df_laps.empty:
        ...
        df_laps = df_laps.sort_values("start_time").reset_index(drop=False)
    ...
    summary = {
        ...
        "date": start_time.date() if start_time else None,  # normalized later
...
if not uploaded:
    ...
    # Несколько файлов → дашборд прогресса (без ICS)
    else:
...
def format_duration(seconds):
    """Из секунд → 'Ч:ММ:СС' (или 'М:СС' если <1 ч). Вернёт None, если невалидно."""
    if seconds is None:
        return None
    try:
...
# ========================= FIT parsing =========================
def parse_fit_file(uploaded_file, hr_rest, hr_max):
    fit = FitFile(uploaded_file)
    ...
    if not df_rec.empty:
        ...
        if df_rec["timestamp"].notna().any():
            ...
            df_rec["t_rel_s"] = np.arange(len(df_rec))
        df_rec["dt_s"] = df_rec["t_rel_s"].diff().fillna(0).clip(lower=0)
        df_rec["pace"] = df_rec["speed"].apply(pace_from_speed)
...
# ========================= Main =========================
def main():
    st.set_page_config(page_title="CapyRun — FIT Analyzer v3.4", page_icon="🏃", layout="wide")
    st.title("🏃 CapyRun — FIT Analyzer v3.4")
    ...
    with st.sidebar:
        ...
        hr_rest = st.number_input("Пульс в покое (HRrest)", min_value=30, max_value=100, value=60, step=1)
        hr_max  = st.number_input("Макс. пульс (HRmax)", min_value=140, max_value=220, value=190, step=1)
        zone_bounds_text = st.text_input("Границы зон HR (уд/мин, через запятую)", value="120,140,155,170,185")
        st.caption("Пример: 120,140,155,170,185 → получится Z1…Z5")
    ...
    if len(uploaded) == 1:
        ...
        with c2:
            st.metric("Время", summary["time_hms"] if summary["time_hms"] else "—")
        with c3:
    ...
    # нормализуем ключ даты и выводим время в человекочитаемом виде
    df_sum["date"] = pd.to_datetime(df_sum["date"]).dt.normalize()
...
def parse_fit_file(uploaded_file, hr_rest, hr_max):
    ...
    for m in fit.get_messages("record"):
        rec_rows.append({
            "timestamp": get_val(m, "timestamp"),
            "hr":        get_val(m, "heart_rate"),
            "speed":     get_val(m, "speed", "enhanced_speed"),
            "cadence":   get_val(m, "cadence"),
            "power":     get_val(m, "power"),
            "elev":      get_val(m, "altitude", "enhanced_altitude"),
            "dist":      get_val(m, "distance"),
        })
    df_rec = pd.DataFrame(rec_rows)
    if not df_rec.empty:
        df_rec["timestamp"] = pd.to_datetime(df_rec["timestamp"], errors="coerce")
...
# ---------------- Main ----------------
def main():
    st.set_page_config(page_title="CapyRun — FIT Analyzer v3.3", page_icon="🏃", layout="wide")
    st.title("🏃 CapyRun — FIT Analyzer v3.3")
    st.caption("Загрузи один или несколько .fit → отчёт / прогресс / план + календарь (ICS) + Excel")
    ...
    if len(uploaded) == 1:
        ...
        with c1:
            st.metric("Дистанция", f"{summary['distance_km']:.2f} км" if summary["distance_km"] else "—")
        with c2:
            st.metric("Время", f"{summary['time_min']:.1f} мин" if summary["time_min"] else "—")
        with c3:
            st.metric("TRIMP (≈)", f"{summary['TRIMP']}" if summary["TRIMP"] else "—")
...
def compute_trimp_timeweighted(hr, dt_seconds, hr_rest, hr_max):
    ...
    # Преобразуем в Series если это не так
    if not isinstance(hr, pd.Series):
        hr = pd.Series(hr)
    if not isinstance(dt_seconds, pd.Series):
        dt_seconds = pd.Series(dt_seconds)
...
def compute_trimp_timeweighted(hr, dt_seconds, hr_rest, hr_max):
    ...
    if isinstance(hr, pd.Series):
        if hr.isna().all(): return None
    if isinstance(dt_seconds, pd.Series):
        if dt_seconds.isna().all(): return None
    rel = (hr - hr_rest) / max(1, (hr_max - hr_rest))
    rel = pd.Series(rel).clip(lower=0)
    minutes = pd.Series(dt_seconds).fillna(0) / 60.0
    val = float((rel * minutes).sum() * 100.0)
    return val if val > 0 else None

def efficiency_factor(speed, hr):
    if speed is None or hr is None: return None
    speed = pd.Series(speed); hr = pd.Series(hr)
    valid = speed.notna() & hr.notna() & (hr > 0)
...
def zones_time(series, bounds):
    if series is None or not bounds: return None
    series = pd.Series(series)
...
def to_excel(dfs_named: dict):
    ...
    with pd.ExcelWriter(bio, engine="xlsxwriter", datetime_format="yyyy-mm-dd hh:mm:ss") as writer:
        for name, df in dfs_named.items():
            ...
            for i, col in enumerate(df.columns):
                try:
                    sample = df[col].head(200).fillna("").astype(str).tolist()
                except Exception:
                    sample = []
                maxlen = min(60, max(len(str(col)), *(len(s) for s in sample)) if sample else len(str(col)))
                ws.set_column(i, i, max(9, maxlen + 1))
...
def ewma_daily(load, tau_days):
    """Exponentially weighted moving average over daily sequence."""
    alpha = 1 - exp(-1.0 / tau_days)
    out = []
    prev = 0.0
    for v in load:
...
if not uploaded:
    ...
    # Несколько файлов → дашборд прогресса
        ...
        # Нормализуем тип ключа к datetime64[ns]
        df_sum["date"] = pd.to_datetime(df_sum["date"]).dt.normalize()
        st.dataframe(df_sum)
        ...
        plan_df = pd.DataFrame()   # гарантируем существование
        note = None
